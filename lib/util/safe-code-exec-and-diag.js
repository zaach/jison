...
//
// - options.dump_sourcecode_on_failure -- default: FALSE
// - options.throw_error_on_failure     -- default: FALSE
//
...
function exec_and_diagnose_this_stuff(sourcecode, code_execution_rig, options, title) {
    ...
    try {
        ...
        if (options.dump_sourcecode_on_failure) {
            try {
                ...
                dumpfile = path.normalize(dumpPath + '/' + dumpName + '.fatal_parser_dump_' + tm + '.js');
        ...
        if (options.throw_error_on_failure) {
            throw ex;
        }
    }
    return p;
