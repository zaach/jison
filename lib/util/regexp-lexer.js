...
function generateRegexesInitTableCode(opt) {
    ...
    a = a.map(function generateXRegExpInitCode(re) {
        console.log('xregexp: ', {
            regex: re, 
            xregexp_data: re.xregexp,
            is_xregexp: re instanceof XRegExp,
            to_string: String(re),
            is_native: re.xregexp.isNative,
            options: opt.options
        });
        if (re instanceof XRegExp) {
            ...
            var s = 'new XRegExp("' + re.xregexp.source.replace(/[\\"]/g, '\\$&') + '", "' + re.xregexp.flags + '")';
            console.log('XREGEXP sourceode: ', s);
            // throw s;
            return s;
        } else {
            return re;
...
function generateModuleBody(opt) {
    ...
    if (opt.rules.length > 0 || opt.in_rules_failure_analysis_mode) {
        ...
        console.log('generate LEXER OPTIONS:', {
            options: opt.options
        });
        if (opt.options) {
