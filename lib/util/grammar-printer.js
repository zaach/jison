...
function grammarPrinter(raw, options) {
    ...
    if (raw.lex) {
        ...
        if (src) {
            for (var i = 0, len = src.length; i < len; i++) {
                ...
                if (key.length <= 12) {
                    if (!actionHasLF) {
                        lex_rules.push(padRight(key, 16) + action);
                    }
                    else {
                        lex_rules.push(padRight(key, 16) + indentAction('{\n' + action + '\n}', 16));
                    }
                }
                else {
                    lex_rules.push(key, indentAction(makeIndent(16) + action, 16));
                }
            }
        }
        ...
        if (src) {
            lex_post.push(indentAction(src, 0));
        }
    ...
    if (src && !isEmptyObj(src)) {
        for (key in src) {
            ...
            default:
                if (value !== true) {
                    grammar_pre.push('%options ' + key + '=' + value);
                }
                else {
                    grammar_pre.push('%options ' + key);
                }
                break;

            case 'ebnf':
                if (value) {
                    grammar_pre.push('%ebnf');
                }
                break;

            case 'type':
                if (value) {
                    grammar_pre.push('%parser-type ' + value);
                }
                break;

            case 'debug':
                if (typeof value !== 'boolean') {
                    grammar_pre.push('%debug ' + value);
                }
                else if (value) {
                    grammar_pre.push('%debug');
                }
                break;
            }
        }
    ...
    if (src) {
        grammar_post.push(indentAction(src, 0));
    }
    ...
    return json5.stringify(raw, null, 2);
}
