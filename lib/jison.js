...
function removeUnusedKernelFeatures(parseFn, info) {
    ...
    if (info.options.noDefaultAction) {
         ...
         *     yyval.$ = vstack[stack.length - len]; // default to $$ = $1
         */
        parseFn = parseFn
        .replace(/\s+\/\/ perform semantic action\n\s+yyval\.\$ = vstack\[stack\.length - [^\n]+\n/g, '\n\n');
    }
...
parser.parse = function parse(input) {
    ...
    stack[sp] = undefined;
    ++sp;
    popStack(stack.length - sp);
    if (sp !== stack.length) { throw new Error('SP != LENGTH'); }
...
_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 1: 
                ...
                lstack.push(lexer.yylloc);
                sstack.push(newState); // push state
		++sp;
                if (sp !== stack.length) { throw new Error('SP != LENGTH'); }
                symbol = null;
            ...
            case 2:
                ...
                // perform semantic action
                yyval.$ = len ? vstack[stack.length - len] : undefined; // default to $$ = $1; result is `undefined` when len == 0, hence there's no $1 (you're reducing an epsilon rule!)
                if (sp !== stack.length) { throw new Error('SP != LENGTH'); }
		...
		sp -= len;
                ...
                newState = table[sstack[stack.length - 2]][stack[stack.length - 1]];
                sstack.push(newState);
		++sp;
                if (sp !== stack.length) { throw new Error('SP != LENGTH'); }
