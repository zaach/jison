        ...
        if (handle.constructor === Array) {
            ...
            if (typeof handle[1] === 'string') {
                ...
                if (action.match(/[$@#][\w][\w\d]*/u)) {
                    ...
                    action = action.replace(/\$([\w][\w\d]*)\b/gu, function (str, pl) {
                            return names[pl] ? '$' + names[pl] : str;
                        }).replace(/@([\w][\w\d]*)\b/gu, function (str, pl) {
                            return names[pl] ? '@' + names[pl] : str;
                        }).replace(/#([\w][\w\d]*)\b/gu, function (str, pl) {
                            return names[pl] ? '#' + names[pl] : str;
                        });
            ...
            handle = handle.replace(/\[[\w][\w\d]*\]/gu, '');
...
parser.parse = function parse(input) {
    ...
    if (this.options.debug) {
        yydebug = function yydebug_impl(msg, obj) {
            ...
            var js;
            try {
                js = JSON.stringify(obj, null, 2).replace(/  "([\w][\w\d]*)": /gu, '  $1: ').replace(/[\n\s]+/g, ' ');
            } catch (ex) {
                js = String(obj);
            }
            self.trace(msg, js, '\n');
