...
function define_parser_APIs_1() {
    return {
        ...
        // Return a more-or-less human-readable description of the given symbol, when available and `return_symbol_identifier_only` is falsey,
        // or the symbol itself, serving as its own 'description' for lack of something better to serve up.
        //
        // The `return_symbol_identifier_only` parameter (default: falsey) is available so userland code can
        // use this API in a grammar parse debugging environment, where you want the symbol *identifier* 
        // instead of its description.
        //
        // Return NULL when the symbol is unknown to the parser.
        describeSymbol: function parser_describeSymbol(symbol, return_symbol_identifier_only) {
            if (return_symbol_identifier_only) {
...
//_lexer_without_token_stack:
    ...
    function lex(parseParams) {
        ...
        if (typeof Jison !== 'undefined' && Jison.lexDebugger) {
            var tokenName = self.describeSymbol(token, true);
