...
Parser.processGrammer = function processGrammerDef(grammer) {
    ...
    this.buildProductions(grammer.bnf, productions, nonterminals, symbols, operators);
...
Parser.augmentGrammer = function augmentGrammer(grammer) {
    ...
    this.startSymbol = grammer.startSymbol || this.productions[0].symbol;
...
Parser.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators) {
    ...
    for (var symbol in bnf) {
        ...
        if (typeof bnf[symbol] === 'string') {
            bnf[symbol] = bnf[symbol].split(/\s*\|\s*/g);
        }
        bnf[symbol].forEach(function (handle) {
            var r;
            if (handle.constructor === Array) {
                if (typeof handle[0] === 'string')
                    handle[0] = handle[0].trim().split(' ');
                ...
                if (typeof handle[1] === 'string' || handle.length == 3) {
                    ...
                    r = new Production(symbol, handle[0], productions.length+1);
...
LookaheadMixin.followSets = function followSets() {
    ...
    nonterminals[this.startSymbol].follows.push(this.EOF);
    ...
    while(cont) {
        ...
        productions.forEach(function (production, k) {
            //this.log(production.symbol,nonterminals[production.symbol].follows);
            var set = [];
...
LRParserMixin.Item = typal.construct({
    constructor: function Item(production, dot, f) {
...
LRParserMixin.ItemSet = typal.construct(Set.prototype, {
    constructor: function ItemSet_(set, raw) {
...
LRParserMixin.gotoOperation = function gotoOperation(itemSet, symbol) {
    ...
    itemSet.forEach(function (item) {
        if (item.markedSymbol() == symbol && symbol != EOF) {
            gotoSet.push(new self.Item(item.production, item.dotPosition+1, item.follows));
        }
    });
...
LRParserMixin.canonicalCollection = function canonicalCollection() {
    var firstState = this.closureOperation(new this.ItemSet(new this.Item(this.productions[0], 0, new Set(this.EOF)))),
    ...
    while (marked !== states.size()) {
        ...
        itemSet.forEach(function (item) {
            if(item.markedSymbol())
                self.canonicalCollectionInsert(item.markedSymbol(), itemSet, states);
        });
    }
    this.log("Canonical");
...
LRParserMixin.canonicalCollectionInsert = function canonicalCollectionInsert(symbol, itemSet, states) {
...
function resolveConflict(production, op, reduce, shift) {
    ...
    if (shift[0] === 'r') {
        sln.msg = "Resolve R/R conflict (use first production declared in grammer.)";
...
var lr0Parser = Parser.beget(LRParserMixin, {
    type: "LR(0)",
    ...
    // Option to compute lookaheads (not needed for LR(0) parsing)
    computeLookAheads: function () {
        this.mix(LookaheadMixin);
        this.nullableSets();
        this.firstSets();
        this.followSets();
    }
...
var LR0Parser = exports.LR0Parser = lr0Parser.construct();
...
/*
 * NQLALR(1) Parser
 * A very naive attempt at an LALR(1) algorithm
 * */
var NQLALRParser = LRLookaheadParser.construct({
    type: "NQLALR(1)",
    ...
    ItemSet: LRParserMixin.ItemSet.prototype.construct({
        closureFilter: function (closureSet) {
    ...
    canonicalCollectionInsert: function (symbol, itemSet, states) {
        var g = this.gotoOperation(itemSet, symbol);
        // add g to que if not empty or duplicate
        if (!g.isEmpty()) {
            ...
            if (i !== -1) {
                itemSet.edges[symbol] = i; // store goto transition for table
                states.item(i).forEach(function (item, n) {
...
var LR1Parser = LRLookaheadParser.construct({
    ...
    Item: LRParserMixin.Item.prototype.construct({
        ...
        toString: function () {
            var temp = this.production.handle.slice(0);
            temp[this.dotPosition] = '.'+(temp[this.dotPosition]||'');
            return '['+this.production.symbol+" -> "+temp.join(' ')+", "+this.follows.join('/')+']';
        }
    }),
...
/*
 * LALR(1) Parser that minimizes LR(1)
 * */
var LALRParser = LR1Parser.prototype.construct({
    beforeconstructor: function () {
        // no-op, divert till after fold
        //this.parseTable = function () { return []; };
    },
    afterconstructor: function () {
        this.minimizeLR();
    },
    type: "LALR(1)",
    // fold an LR table when states only differ by lookahead
    minimizeLR: function () {
        var eq = this.Item.prototype.eq;
        var self = this;
        // set eq to version that ignores lookahead
        this.Item.prototype.eq = LR0Parser.prototype.Item.prototype.eq;
        var edges = {};
        var newStates = new Set();

        this.states.forEach(function (state, k) {
            var i = newStates.indexOf(state);
            if (i === -1) {
                edges[k] = newStates.size();
                newStates.push(state);
            } else {
                // merge states
                newStates.item(i).forEach(function (item, n) {
                    item.follows.joinSet(state.item(n).follows);
                });
                // store translation, old points to new
                edges[k] = i;
                self.log("Merge states: "+k+", "+i);
            }
        });
        this.Item.prototype.eq = eq; // reset eq

        // translate edges
        newStates.forEach(function (state) {
            for (var symbol in state.edges) {
                state.edges[symbol] = edges[state.edges[symbol]];
            }
        });
        this.LRStates = this.states;
        this.states = newStates;

        // build parse table
        this.table = this.constructor.prototype.parseTable.call(this, this.states);
    }
});
...
var LLParser = Parser.construct(LookaheadMixin, {
    ...
    parseTable: function llParseTable(productions) {
        ...
        productions.forEach(function (production, i) {
            ...
            (production.nullable ? self.nonterminals[production.symbol].follows : production.first).forEach(function (token) {
                if (row[token]) {
                    row[token].push(i);
                    self.conflicts++;
                } else {
