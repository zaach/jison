...
var code_exec  = require('./util/safe-code-exec-and-diag.js').exec;
...
function mkStdOptions(/*args...*/) {
    ...
    for (var i = 0, len = arguments.length; i < len; i++) {
        ...
        for (var p in o2) {
            if (h.call(o2, p)) {
                if (typeof o2[p] !== 'undefined' && o2[p] !== Jison.defaultJisonOptions[p]) {
                    opts[p] = o2[p];
                }
            }
        }
    ...
    opts.defaultActionMode = opts.defaultActionMode.slice(0);
...
generator.buildProductionActions = function buildProductionActions() {
    ...
    */
    ...
    while (gen_hash !== prev_gen_hash) {
        ...
        this.actionsUseYYERROR = this.actionsUseYYERROR || analyzeFeatureUsage(this.performAction, /\b\.yyError\b/g, 0);
        this.actionsUseYYRECOVERING = this.actionsUseYYRECOVERING || analyzeFeatureUsage(this.performAction, /\b\.yyRecovering\b/g, 0);
        this.actionsUseYYERROK = this.actionsUseYYERROK || analyzeFeatureUsage(this.performAction, /\b\.yyErrOk\b/g, 0);
        this.actionsUseYYCLEARIN = this.actionsUseYYCLEARIN || analyzeFeatureUsage(this.performAction, /\b\.yyClearIn\b/g, 0);
        ...
        this.actionsUseLocationTracking = this.actionsUseLocationTracking || this.actionsUseYYLINENO || this.actionsUseYYLOC || this.actionsUseLocationAssignment;
    ...
    function buildProductionAction(handle) {
        ...
        case 0:
            ...
            case "merge":
                ...
                // @$ = yyparser.mergeLocationInfo(null, null, null, null, true);
                default_action.push("this._$ = yyparser.mergeLocationInfo(null, null, null, null, true);");
                break;
        ...
        case 1:
            ...
            case "classic":
                // $$ = $1;
                default_action.push("this.$ = yyvstack[yysp" + indexToJsExpr(1, rhs.length, rule4msg) + "];");
                break;

            case "ast":
                // bundle all production terms in an array:
                // $$ = yyvstack.slice(yysp - ${rhs.length - 1}, yysp + 1);
                default_action.push("this.$ = [yyvstack[yysp" + indexToJsExpr(rhs.length, rhs.length, rule4msg) + "]];");
                break;
            ...
            case "merge":
                ...
                // @$ = @1;
                default_action.push("this._$ = yylstack[yysp" + indexToJsExpr(1, rhs.length, rule4msg) + "];");
                break;
        ...
        default:
            ...
            case "ast":
                // bundle all production terms in an array:
                // $$ = yyvstack.slice(yysp - ${rhs.length - 1}, yysp + 1);
                default_action.push("this.$ = yyvstack.slice(yysp" + indexToJsExpr(rhs.length, rhs.length, rule4msg) + ", yysp + 1);");
                break;
            ...
            case "merge":
                // merge all production terms' locations into a single range:
                // @$ = yyparser.mergeLocationInfo(yysp - ${rhs.length - 1}, yysp);
                default_action.push("this._$ = yyparser.mergeLocationInfo(yysp" + indexToJsExpr(1, rhs.length, rule4msg) + ", yysp);");
                break;
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    if (yydebug) yydebug('Error recovery process: performAction: COMBINE: ', {
                        yyval, yytext, sp, len, vstack, stack, sstack,
                        combineState: NO_ACTION[1]
                    });
                    ...
                    for (var idx = sp - EXTRA_STACK_SAMPLE_DEPTH, top = idx + len; idx < top; idx++, esp++) {
