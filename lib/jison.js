    ...
    try {
        ...
        try {
            ...
            var tm = ts.getUTCFullYear() +
                ...
                'Z';
            var dumpfile = path.normalize(dumpPath + '/' + dumpName + '.fatal_parser_dump_' + tm + '.js');
            console.error("****** offending generated parser source code dumped into file: ", dumpfile);
            fs.writeFileSync(dumpfile, sourcecode, 'utf8');
        } catch (ex2) {
            console.error("generated parser source code fatal DUMPING error: ", ex2.message, ex2.stack);
        }
        throw ex;
    }
