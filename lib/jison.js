...
Parser.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators) {
    var actions = [this.actionInclude || "", 'switch(arguments[3]) {'],
        prods, symbol;
    var productions_ = [0];
    var symbolId = 1;
    var symbols_ = {};
    ...
    for (symbol in bnf) {
        ...
        addSymbol(symbol);
        nonterms_[symbol] = symbols_[symbol];
            ...
            if (handle.constructor === Array) {
                ...
                if (typeof handle[1] === 'string' || handle.length == 3) {
                    ...
                    action = action.replace(/\$(?:0|\$)/g, "this.yyval")
                        .replace(/\$(\d+)/g, "arguments[4][arguments[4].length-"+rhs.length+"+$1-1]");
                    actions.push(action);
    ...
    itemSets.forEach(function (itemSet, k) {
            ...
            itemSet.forEach(function (item, j) {
                ...
                if (item.markedSymbol() == stackSymbol) {
                    ...
                    if (nonterminals[stackSymbol]) {
                        ...
                        state[stackSymbol] = gotoState; 
                    } else {
                        self.log(k, stackSymbol, action);
                        state[stackSymbol] = [['s',gotoState]];
                    }
                }
            });
        }
        ...
        itemSet.reductions.forEach(function (item, j) {
            ...
            terminals.forEach(function (stackSymbol) {
                action = state[stackSymbol] || [];
                var op = operators[stackSymbol];
                // Reading a terminal and current position is at the end of a production, try to reduce
                if (action.length) {
                    var r = resolveConflict(item.production, op, ['r',item.production.id], action[0]);
...
LRParserMixin.parse = function parse(input) {
    ...
    while (true) {
        ...
        if (typeof action == 'undefined' || !action.length || !action[0]) {
            expected = [];
            for (p in table[state]) if (!this.nonterminals_[p] && p !== this.EOF) {
                expected.push("'"+p+"'");
            }
        ...
        switch (a[0]) {
            ...
            case 'r': // reduce
                ...
                stack.push(this.symbols[this.productions_[a[1]][0]]);    // push nonterminal (reduce)
...
LRParserMixin.generateModule_ = function generateModule () {
    ...
    out += [
        ...
        "nonterminals_: " + JSON.stringify(this.nonterminals_),
...
// default mian method for generated commonjs modules
...
exports.main = function main (args) {
    ...
    stream.print(parser.generate(opt));
    stream.close();
};
