...
_handle_error_with_recovery:                    // run this code when the grammar includes error recovery rules
    ...
    function locateNearestErrorRecoveryRule(state) {
        ...
        for (;;) {
            ...
            // note that the `table[state][TERROR]` entry MAY have the value 0 (zero) to 
            // signal `$accept` for the given state; hence we should compare against 
            // `undefined` explicitly.
            if (table[state][TERROR] !== undefined) {
                return depth;
            }
            if (state === 0 /* $accept */ || stack_probe < 2) {
...
_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 2:
                ...
                if (newState.length) {
                    console.log('unexpected GOTO table blob instead of basic action number', {
                        state: stack[stack.length - 2],
