...
var printer    = require('./util/grammar-printer.js');
var rmCommonWS = printer.rmCommonWS;
...
Jison.JSON5 = json5;
Jison.prettyPrint = printer.prettyPrint;
...
function removeUnusedKernelFeatures(parseFn, info) {
    ...
    if (info.options.noTryCatch) {
         ...
         */
        parseFn = parseFn
        .replace(/\s+try \{([\s\r\n]+this\.__reentrant_call_depth[\s\S]+?)\} catch \(ex\) \{[\s\S]+?\} finally \{([^\}]+)\}/, function replace_noTryCatch(m, p1, p2) {
            p1 = p1.replace(/^        /mg, '    ');
            p2 = p2.replace(/^        /mg, '    ');
            return '\n' + p1 + '\n    // ... AND FINALLY ...\n' + p2;
        })
