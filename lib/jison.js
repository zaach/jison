...
lrGeneratorMixin.generateTableCode2 = function (table, defaultActions, productions) {
    ...
    var compressProductions = (productionsDef.pop.length > 5);
    var compressDefaultActions = (defaultActionsDef.pop.length > 5);
    var compressGotoTable = (tableDef.symbol.length > 10);
    var compressAnything = (compressProductions || compressDefaultActions || compressGotoTable);

    tableCode = reportColumnsForCompression(tableDef) + (compressGotoTable ? generateCompressedGotoTable(tableDef) : tableCode);
    defaultActionsCode = reportColumnsForCompression(defaultActionsDef) + (compressDefaultActions ? generateCompressedSet(defaultActionsDef) : defaultActionsCode);
    productionsCode = reportColumnsForCompression(productionsDef) + (compressProductions ? generateCompressedTable(productionsDef) : productionsCode);
