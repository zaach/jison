    ...
    if (!this.hasErrorRecovery) {
        var chk_er_opt = function check_error_recovery_option(opt, label) {
            if (self.options[opt]) {
                ...
                self.warn('The grammar does not have any error recovery rules, so use the ' + label + ' is rather useless.');
            }
        };
...
function preprocessActionCode(s) {
    function replace_markers(cmt) {
        ...
        .replace(/\byy([a-zA-Z]+)\b/g, '\x01\x1D__$1');   // `yyxxx`: all `yy`-prefixed (camelCased) identifiers are RESERVED USE for jison.
...
generator.buildProductionActions = function buildProductionActions() {
    ...
    */
    ...
    for (var gen_level = 1, prev_gen_hash = 'n', gen_hash = 'y'; gen_hash !== prev_gen_hash; gen_level++, (prev_gen_hash = gen_hash), (gen_hash = mkParserFeatureHash(this))) {
        ...
        if (this.hasErrorRecovery) {
            var err_recov_id = stateHasAction.length;
            var userland_err_recov_redux_code = '';

            actions.push(`case ${err_recov_id}:
        ...
        this.actionsUseYYERROR = this.actionsUseYYERROR || analyzeFeatureUsage(this.performAction, /\byyerror\b/g, 0);
        this.actionsUseYYRECOVERING = this.actionsUseYYRECOVERING || analyzeFeatureUsage(this.performAction, /\bYYRECOVERING\b/g, 0);
        this.actionsUseYYERROK = this.actionsUseYYERROK || analyzeFeatureUsage(this.performAction, /\byyerrok\b/g, 0);
        this.actionsUseYYCLEARIN = this.actionsUseYYCLEARIN || analyzeFeatureUsage(this.performAction, /\byyclearin\b/g, 0);
    ...
    function buildProductionAction(handle) {
        ...
        if (handle.action) {
            ...
            var named_token_re = new XRegExp('^[\\p{Alphabetic}_][\\p{Alphabetic}_\\p{Number}]*$');   // WARNING: this regex MUST match the regex for `ID` in ebnf-parser::bnf.l jison language lexer spec! (`ID = [{ALPHA}]{ALNUM}*`)
            ...
            if (!self.options.noDefaultAction) {
                // no user action specified, hence insert default action:
                ...
                case 0:
                    action = "$$ = undefined;";
                    if (self.actionsUseYYLOC || self.actionsUseLocationTracking || self.actionsUseLocationAssignment) {
                        action += "\n@$ = undefined;";
                    }
                    break;
                ...
                default:
                    action = '$$ = [' + rhs.map(function (sym, idx) {
                        return '$' + (idx + 1);
                    }).join(', ') + '];';
                    if (self.actionsUseYYLOC || self.actionsUseLocationTracking || self.actionsUseLocationAssignment) {
                        var first_valid_loc_entry = rhs.map(function (term, idx) {
                            return '@' + (idx + 1);
                        });
                        var last_valid_loc_entry = first_valid_loc_entry.slice(0).reverse();

                        action += `
                            var __l0 = ${first_valid_loc_entry.join(' || ')}; 
                            var __l1 = ${last_valid_loc_entry.join(' || ')};
                            if (__l0 /* && __l1 */) { 
                                @$ = {
                                    first_line: __l0.first_line,
                                    last_line: __l1.last_line,
                                    first_column: __l0.first_column,
                                    last_column: __l1.last_column
                                };
                                if (__l0.range && __l1.range) {
                                    @$.range = [__l0.range[0], __l1.range[1]];
                                }
                            } else {
                                @$ = undefined;
                            }
                ...
                // when the option noDefaultAction has been set, we still strive to produce
                // a deterministic output, hence we take the swiftest route toards producing
            ...
            handle.action = '// default action (generated by JISON):\n' + action;

            buildProductionAction(handle);
...
lrGeneratorMixin.generateModule_ = function generateModule_() {
    ...
    var rulesLst = ((this.options.outputDebugTables || this.options.exportAllTables) ? JSON.stringify(rules4Dbg, null, 2) : undefined);
    if (rulesLst) {
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    r = this.performAction.call(yyval, yytext, yyleng, yylineno, yyloc, NO_ACTION[1], sp - 1, len, vstack, lstack, stack, sstack);
...
//_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 2:
                ...
                r = this.performAction.call(yyval, yytext, yyleng, yylineno, yyloc, newState, sp - 1, len, vstack, lstack, stack, sstack);
