...
function rmCommonWS(strings, ...values) {
    ...
    // that would be the SECOND/THIRD/... line of input, always, as the FIRST line won't
    // have any indentation at all!
    var s0 = '';
outerloop:
    for (var i = 0, len = src.length; i < len; i++) {
        for (var j = 1, cnt = src[i].length; j < cnt; j++) {
            s0 = src[i][j].trim();
            if (s0 !== '') {
                break outerloop;
            }
...
//_lexer_without_token_stack:
    ...
    function lex() {
        ...
        if (self.terminals_) console.log("### lexed token: ", token, self.terminals_[token]);
        return token || EOF;
    }
...
//_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action) {
                    ...
                    if (error_rule_depth < 0) {
                        if (typeof assert !== 'undefined') assert(recovering > 0);
                        recoveringErrorInfo.info_stack_pointer = esp;
                    ...
                    if (typeof assert !== 'undefined') assert(symbol === TERROR);
                    while (symbol) {
                        // retrieve state number from top of stack
                        state = newState;               // sstack[sp - 1];
...
if (0) {
    console.log("#################### error recovery start: ", {
        ...
        value: vstack[sp],
        loc: lstack[sp],
        locstart: recoveringErrorInfo.loc, 
        locend: lexer.yylloc,
    });                        
} else {
    console.log("#################### error recovery start! ");
...
//_handle_error_end_of_section:                  // this concludes the error recovery / no error recovery code section choice above
            ...
            case 1:
                ...
                ++sp;
                symbol = 0;
                if (typeof assert !== 'undefined') assert(preErrorSymbol === 0);
                if (!preErrorSymbol) { // normal execution / no error
                    // Pick up the lexer details for the current symbol as that one is not 'look-ahead' any more:
                    yyleng = lexer.yyleng;
                    yytext = lexer.yytext;
