...
function findDefaults(states) {
    ...
    states.forEach(function (state, k) {
        ...
        if (i === 1 && state[act][0] === 2) {
            console.error('###### findDefaults A: ', k, state);
            // only one action in state and it's a reduction/shift/accept
            defaults[k] = state[act];
        } else if (i === 1) {
            console.error('###### findDefaults B: ', k, state);
        }
    });
...
parser.collect_expected_token_set = function collect_expected_token_set(state) {
...
_handle_error_with_recovery:                // run this code when the grammar includes error recovery rules
                ...
                if (!action || !action.length || !action[0]) {
                    ...
                    if (recovering === 3) {
                        ...
                        if (error_rule_depth < 0 && (symbol === EOF || preErrorSymbol === EOF)) {
