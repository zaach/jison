...
parser.parse = function parse(input, parseParams) {
    ...
    if (this.options.debug) {
        yydebug = function yydebug_impl(msg, obj) {
            ...
            obj.symbol_stack = [].concat(stack);
            obj.state_stack = [].concat(sstack);
            obj.value_stack = [].concat(vstack);
            obj.location_stack = [].concat(lstack);
            obj.stack_pointer = sp;

            // clip unused tail from these arrays to improve debug display:
            for (var ll = sp + 1; obj.symbol_stack[ll] !== undefined; ll++)
                ;
            obj.clipped_length = ll;

            obj.symbol_stack.length = ll;
            obj.state_stack.length = ll;
            obj.value_stack.length = ll;
            obj.location_stack.length = ll;
    ...
    this.cleanupAfterParse = function parser_cleanupAfterParse(resultValue, invoke_post_methods) {
        ...
        // Userland code therefor cannot depend on its lifetime reaching 
        // beyond the `parse()` run itself and must COPY any data/references
    ...
    this.constructParseErrorInfo = function parser_constructParseErrorInfo(msg, ex, expected, recoverable) {
        // clip unused tail from these arrays to improve debug display:
        for (var ll = sp + 1; stack[ll] !== undefined; ll++)
            ;
        stack.length = ll;
        sstack.length = ll;
        vstack.length = ll;
