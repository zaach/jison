...
generator.buildProductionActions = function buildProductionActions() {
    ...
    */
    ...
    for (var gen_level = 1; gen_level <= 2; gen_level++) {
        var actions = [
          '/* this == yyval */',
        ...
        if (missingActions.length) {
            ...
            actions.push(`default:
                ...
                function __b0rk_on_internal_failure(str) {
                    var self = yy.parser;
                    var hash = self.constructParseErrorInfo(str, null, null, false);

                    var r = self.parseError(str, hash, self.JisonParserError);
                    return r;
                }

                return __b0rk_on_internal_failure("internal parser failure: resolving unlisted state: " + yystate /* , yyvstack[yysp] */ );`
            );
        }
        actions.push('}');
        ...
        this.actionsUseYYRULELENGTH |= analyzeFeatureUsage(this.performAction, /\byyrulelength\b/g, 1);
    ...
    function buildProductionAction(handle, index) {
        ...
        if (handle.action) {
                ...
                action = action.replace(
                    new XRegExp('([$@#]|##)([\\p{Alphabetic}_][\\p{Alphabetic}_\\p{Number}]*)', 'g'), function (str, mrkr, pl) {
                        if (names[pl] && count[pl] !== 1) {
                            throw new Error('The action block references the ambiguous named alias or term reference "' + pl + '" ' +
                                            'which is mentioned ' + count[pl] + ' times in production "' + handle.handle + '", implicit and explicit aliases included.\n' +
            ...
            buildProductionAction(handle, index);
