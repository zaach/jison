...
LRParserMixin.canonicalCollection = function canonicalCollection() {
    var items = this.closureOperation(new this.ItemSet(new this.Item(this.productions[0], 0, new Set(this.EOF)))),
        sets = new Set(items),
        done = new Set(),
        self = this,
        itemSet;
    ...
    while (!sets.isEmpty()) {
        ...
        this.symbols.forEach(function (symbol) {
            self.canonicalCollectionInsert(symbol, itemSet, done, sets);
        });
    }
    this.log("Canonical");
...
LRParserMixin.canonicalCollectionInsert = function canonicalCollectionInsert(symbol, itemSet, done, sets) {
    var g = this.gotoOperation(itemSet, symbol);
    // add g to que if not empty or duplicate
    if (!g.isEmpty()) {
        itemSet._goto[symbol] = g; // store goto transition for table
        if (!done.contains(g)) {
            sets.push(g); 
        }
    }
...
var LALRParser = LRLookaheadParser.construct({
    ...
    canonicalCollectionInsert: function (symbol, itemSet, done, sets) {
        var g = this.gotoOperation(itemSet, symbol);
        // add g to que if not empty or duplicate
        if (!g.isEmpty()) {
            itemSet._goto[symbol] = g; // store goto transition for table
