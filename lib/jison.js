...
generator.buildProductions = function buildProductions(bnf, productions, nonterminals, symbols, operators, predefined_symbols, descriptions) {
    ...
    function indexToJsExpr(n, len, rule4msg) {
        var v = parseInt(n, 10) - len;
        // the usual situation: $3 ==> 'x - 3'
        if (v < 0) {
            return ' - ' + (-v);
        }
        // the VERY UNusual situation: $-1 ==> 'x + 1'
        if (v > 0) {
            return ' + ' + v;
        }
        // decode error?
        if (v !== 0) {
            throw new Error('invalid token reference in rule: "' + rule4msg + '"');
        }
        // the usual situation: $0
        return '';
    }
            ...
            if (typeof handle[1] === 'string') {
                ...
                action = action
                    ...
                    .replace(/#(-?\d+)\b/g, function (_, n) {
                        ...
                        if (!rhs[i]) {
                            throw new Error('invalid token location reference in rule: "' + rule4msg + '"');
                        }
