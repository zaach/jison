...
lrGeneratorMixin.gotoOperation = function gotoOperation (itemSet, symbol, states) {
    ...
    if(states.has[gotoSet.valueOf()] !== undefined){
        return states.has[gotoSet.valueOf()];
    }else{
        states.has[gotoSet.valueOf()] = states.size();
    }
...
lrGeneratorMixin.canonicalCollectionInsert = function canonicalCollectionInsert (symbol, itemSet, states, stateNum) {
    var g = this.gotoOperation(itemSet, symbol, states);

    if(typeof g === 'number'){
        itemSet.edges[symbol] = g; // store goto transition for table
        states.item(g).predecessors[symbol].push(stateNum);
    }else{
        if (!g.predecessors)
            g.predecessors = {};
