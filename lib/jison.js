...
parser.parse = function parse(input) {
    ...
    this.cleanupAfterParse = function parser_cleanupAfterParse(resultValue, invoke_post_methods) {
        ...
        if (invoke_post_methods) {
            if (!sharedState.yy) throw new Error('kaboom');
    ...
    try {
        ...
        for (;;) {
            ...
            if (this.defaultActions[state]) {
                ...
                // error symbol lookup should always deliver a SHIFT (the token *following* the error can then produce a REDUCE on an error rule!)
                if (symbol === TERROR && action === 2) throw new Error('uh-oh! reduce on error token!');
            ...
            case 1:
                ...
                if (!preErrorSymbol) { // normal execution / no error
                    ...
                    if (!t[0]) {
                        ...
                        // recovering from now...
                        if (yydebug) yydebug('... SHIFT:error recovery: re-pplication of old symbol doesn\'t work: instead, we\'re moving forward now. ', { recovering: recovering, symbol: symbol });
                        symbol = 0;
                    }
                }
