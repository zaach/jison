...
lrGeneratorMixin.canonicalCollection = function canonicalCollection() {
    ...
    console.log('canonicalCollection: ', states.has);

    while (marked !== states.size()) {
        itemSet = states.item(marked);
        itemSet.forEach(function CC_itemSet_forEach(item) {
            console.log('canonicalCollection:item Test: ', {
                marked: marked,
                item: item,
                states: states 
            });
            if (item.markedSymbol && item.markedSymbol !== self.EOF) {
                console.log('canonicalCollection:item INSERT', {
                    markedSymbol: item.markedSymbol,
                    index: marked
                });
...
var lalr = generator.beget(lookaheadMixin, generatorMixin, lrGeneratorMixin, {
    ...
    go: function LALR_go(stateId, productionHandle) {
        assert(typeof stateId === 'number');
        for (var i = 0; i < productionHandle.length; i++) {
            stateId = this.states.item(stateId).edges[productionHandle[i]] || stateId;
        }
        return stateId;
    },
    goPath: function LALR_goPath(stateId, productionHandle) {
        assert(typeof stateId === 'number');
        var endStateId = stateId,
            t,
            path = [];
        for (var i = 0; i < productionHandle.length; i++) {
            t = productionHandle[i] ? endStateId + ':' + productionHandle[i] : '';
            if (t) {
                this.newg.nterms_[t] = endStateId;
            }
            path.push(t);
            endStateId = this.states.item(endStateId).edges[productionHandle[i]] || endStateId;
