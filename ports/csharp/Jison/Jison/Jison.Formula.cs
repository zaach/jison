...
namespace jQuerySheet
{
	public class Formula
	{
		...
		public SpreadsheetExpression ParserPerformAction(ref SpreadsheetExpression thisS, ref SpreadsheetExpression yy, ref int yystate, ref JList<SpreadsheetExpression> ss)
		{
			var so = ss.Count - 1;
/* this == yyval */
...
case 23:
				...
				thisS = FormulaFunctions.Call(ss[so-2].Value);
...
case 24:
				...
				thisS = FormulaFunctions.Call(ss[so-3].Value, ss[so-1]);
...
case 28:
				...
				thisS = Spreadsheet.CellValue(CellLocation.ParseFixed(ss[so]));
...
case 29:
				...
				thisS = Spreadsheet.CellValue(CellLocation.ParseFixed(ss[so-2]), CellLocation.ParseFixed(ss[so]));
...
case 30:
				...
				thisS = Spreadsheet.CellValue(CellLocation.Parse(ss[so]));
...
case 31:
				...
				thisS = Spreadsheet.CellValue(CellLocation.Parse(ss[so-2]), CellLocation.Parse(ss[so]));
...
case 32:
				...
				thisS = Spreadsheet.CellValue(CellLocation.ParseRemote(ss[so-2], ss[so]));
...
case 33:
				...
				thisS = Spreadsheet.CellValue(CellLocation.ParseRemote(ss[so-4], ss[so-2]), CellLocation.ParseRemote(ss[so-4], ss[so]));
...
case 41:
				...
				thisS = new T(ss[so-1].ToDouble() * 0.01);
...
break;
		...
		public SpreadsheetExpression Parse(string input)
		{
			if (Table == null) {
			...
			var vstack = new JList<SpreadsheetExpression>
			{
				new SpreadsheetExpression()
			};
			var yy = new SpreadsheetExpression();
			var _yy = new SpreadsheetExpression();
			var v = new SpreadsheetExpression();
			int recovering = 0;
			...
			while (true)
			{
				...
				switch (action.Action)
				{
					...
					case Reduce:
				        ...
				        if (_yy == null)
				        {
							vstack.Push(new SpreadsheetExpression());
				        }
				        else
				        {
		...
		public SpreadsheetExpression Yy = new SpreadsheetExpression();
		public string Match = "";
