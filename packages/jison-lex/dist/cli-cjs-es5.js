...
var nomnom = _interopDefault(require('@gerhobbelt/nomnom')); // Return TRUE if `src` starts with `searchString`. 
...
function updatePcodesBitarrayCacheTestOrder(opts) {
  ...
  for (j = 0; keys[j]; j++) {
    ...
    if (!done[k]) {
      assert$1(l[k] > 0); // find a minimum span character to mark this one:
...
function bitarray2set(l, output_inverted_variant, output_minimized) {
  ...
  assert$1(rv.length);
  var s = rv.join('');
  assert$1(s); // Check if the set is better represented by one of the regex escapes:
...
function reduceRegexToSetBitArray(s, name, opts) {
  ...
  assert$1(s); // propagate deferred exceptions = error reports.
...
function autodetectAndConvertToJSONformat(lexerSpec, options) {
  ...
  if (typeof lexerSpec === 'string') {
    if (options.json) {
      try {
        chk_l = JSON5.parse(lexerSpec); // When JSON5-based parsing of the lexer spec succeeds, this implies the lexer spec is specified in `JSON mode`
      ...
      for (i in macros) {
        if (macros.hasOwnProperty(i)) {
          ...
          if (a.length > 1) {
            ...
            x = m.elsewhere;
            assert$1(x); // detect definition loop:
...
function RegExpLexer(dict, input, tokens, build_options) {
  ...
  function test_me(tweak_cb, description, src_exception, ex_callback) {
    ...
    var source = generateModule(opts); // opts.exportSourceCode.all
    ...
    try {
      ...
      var lexer = code_exec$1(testcode, function generated_code_exec_wrapper_regexp_lexer(sourcecode) {
        chkBugger$3(sourcecode); //babelize the source code for subsequent execution by Node:

        var es5src = helpers.compileCodeToES5(sourcecode); //console.log("===============================LEXER TEST ES5 CODE\n", es5src.code, "\n=====================END====================\n");

        var fcode = es5src.code;
        fcode = fcode.replace(/function\s+xxxxxxxxxxxxxxx\(\)[\r\n\s]*\{[\r\n\s]*['"]use strict['"];/, '').replace(/\}[\r\n\s]*$/, '').replace(/function _typeof\(obj\) \{.*?\}$/m, '').replace(/\b_typeof\(/g, 'typeof (');
  ...
  var lexer = test_me(null, null, null, function (ex) {
    ...
    if (!test_me(function () {
      ...
      if (!test_me(function () {
        ...
        return opts.lex_rule_dictionary.rules.length > 0 ? 'One or more of your lexer state names are possibly botched?' : 'Your custom lexer is somehow botched.';
      }, ex, null)) {
        var rulesSpecSize;
        ...
        if (!test_me(function () {
          ...
          assert$1(Array.isArray(opts.lex_rule_dictionary.rules));
          assert$1(Array.isArray(opts.rules));
          rulesSpecSize = opts.lex_rule_dictionary.rules.length; // opts.conditions = [];
          ...
          for (var i = 0, len = rulesSpecSize; i < len; i++) {
            ...
            rv = test_me(function () {
              assert$1(Array.isArray(opts.lex_rule_dictionary.rules));
              assert$1(opts.lex_rule_dictionary.rules.length === rulesSpecSize); // opts.conditions = [];
              // opts.rules = [];
              // opts.__in_rules_failure_analysis_mode__ = true;
...
function cliMain(opts) {
  ...
  function mkdirp(fp) {
    ...
    try {
      ...
      if (e.code === 'ENOENT') {
        var parent = path.dirname(fp); // Did we hit the root directory by now? If so, abort!
  ...
  function processInputFile() {
    ...
    var raw = fs.readFileSync(path.normalize(opts.file), 'utf8'); // making best guess at json mode
